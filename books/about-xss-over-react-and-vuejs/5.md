...
Vue では HTML コンテンツを自動でエスケープ処理し、誤って実行可能な HTML をアプリケーション内に挿入することを防いでいますが、HTML が安全なことが事前にわかっている場合は明示的にそれをレンダリングすることが可能です。
v-html を使う場合は XSS が刺さりやすいということは広く知られていますが、どちらかというとこれは Vue の innerHTML を使用した場合に広く適用されるケースです。
...
[React の章]((4#JSX を使わないケース(dangerouslySetInnerHTML))でも少し触れましたが、このケースは [element.innerHTML](https://developer.mozilla.org/ja/docs/Web/API/Element/innerHTML) をそのまま使用することに相当します。
...
[公式ドキュメント](https://jp.vuejs.org/v2/guide/security.html#%E3%82%B9%E3%82%BF%E3%82%A4%E3%83%AB%E3%81%AE%E6%8C%BF%E5%85%A5)では続けて `v-bind` におけるクリックジャッキング脆弱性を紹介しているので、興味のある方は是非読んでみてください。
...
是非、[Running the demo](https://github.com/dotboris/vuejs-serverside-template-xss) を参考にセットアップし、実際に試してみてください。
`docker-compose` で直ぐにローカル環境で確認できます。
...
:::message alert
**Spoiler Alert**
やられアプリとして [vuejs-serverside-template-xss](https://github.com/dotboris/vuejs-serverside-template-xss) を試してみたい方は、以下はネタバレ注意です。
:::

ソースコードを見ると、[htmlspecialchars](https://www.php.net/manual/ja/function.htmlspecialchars.php) でエスケープした「安全なデータ」をサーバーサイドから受け取利、クライアントサイドの Vue でそのまま表示していることがわかります。
...
この解決策について、dotboris はより体系的な解決策としてページの中でグローバル変数を定義して、 すべてのサーバ側の変数を保持する方法を紹介しています。
...
ReactやAngular 2+ のようなフレームワークはテンプレートをHTMLで書かせないようにしていますし、この手の攻撃は刺さりにくいです。
